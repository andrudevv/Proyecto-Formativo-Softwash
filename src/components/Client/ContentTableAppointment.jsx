...
export default function ContentTableAppointment({
  fields,
  fieldsMapping,
  data,
  buttonActions,
        ...
        {data.map((row, index) => (
          ...
          >
            {fields.map((field, subIndex) => (
              <td
                key={subIndex}
                className={`text-center  max-w-xl break-all border-collapse border table-cell  border-black p-2 ${
                  field === "acciones" ? styleActions : ""
                }`}
              >
                {field === "acciones"
