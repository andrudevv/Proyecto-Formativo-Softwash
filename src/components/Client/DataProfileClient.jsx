...
//
export default function DataProfileClient({
  client,
  onClick,
  onclickImg,
  fileInputRef,
      ...
      <div className="grid grid-cols-2  gap-10">
        <div className="w-full grid  justify-center col-span-2">
          <img
            src={img}
            alt="img"
            className="rounded-md w-[250px] h-[250px] col-span-1  shadow-lg shadow-black"
          />
          <button
            onClick={onclickImg}
            className=" mt-8 shadow-sm shadow-black font-semibold w-auto hover:opacity-100 rounded-md transition-opacity m-4 ease-in-out duration-300 hover:bg-blue-400 hover:text-white"
          >
            {" "}
            Cambiar foto
          </button>
        ...
        <div className="col-span-2 text-center">
          <ButtonAction
            onClick={onClick}
            estilos={"w-1/3 h-10 font-bold bg-button-primary rounded-md"}
            text={"Actualizar"}
          />
        </div>
      </div>
