...
export default function DataProfileClient({ client, onClick, onclickImg, fileInputRef, handleFileChange }) {
  ...
  useEffect(() => {
    if (client) {
      ...
      // handleHourOpen(clientD.aperture.toLocaleTimeString([],{hour: '2-digit', minute: '2-digit', hour12: true}))
      setLoading(false);
    }
  ...
  return (
    <div className="max-w-2xl mx-auto bg-white p-8 rounded-md shadow-md">
      <div className="grid grid-cols-2  gap-10">
        ...
        <div>
          <strong>Hora Apertura:</strong> {clientD.aperture  } AM
        </div>
        <div>
          <strong>Hora Cierre:</strong> {clientD.closing } PM
        </div>
        <div>
