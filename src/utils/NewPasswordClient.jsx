...
export default function NewPasswordClient() {
    ...
    const { newPasswordClient, registerErrors } = clientAuth();
    ...
    const onSubmitNewPassword = handleSubmit(async (email) => {
        setLoading(true);
        const sendEmail = await newPasswordClient(tokenDes, email);
        if (sendEmail) {
            ...
            {loading ? <Spinner /> : <>
                <div className="flex  items-center justify-center h-screen ">
                    ...
                    <form
                        ...
                        className="bg-gray-200 bg-opacity-75 text-center shadow-xl shadow-blue-500 rounded-lg absolute w-5/12 p-10"
                    >
